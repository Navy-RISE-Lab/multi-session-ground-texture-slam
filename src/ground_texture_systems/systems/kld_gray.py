...
class KLDGray(base.Base):
    ...
    def __init__(self, common_options: base.CommonParameters) -> None:
        ...
        self._baseline_histogram: NDArray = np.zeros(shape=(256,))
    ...
    def insert_frame(
        ...
        if self._current_session == 0:
            self._baseline_images.append(
                cv2.cvtColor(src=image, code=cv2.COLOR_BGR2GRAY),
            )
    ...
    def new_session(self) -> int:
        ...
        if self._current_session == 0:
            ...
            self._baseline_histogram = self._create_histogram(
                image=image_array,
            )
    ...
    def _evaluate_loop(self, current_index: int, image: NDArray) -> None:
        ...
        for candidate_index, bow_score in sorted(
            ...
            try:
                ...
                # Use KLD to adjust the score if the textures are different.
                if self._current_session > 0:
                    image_histogram = self._create_histogram(
                        image=cv2.cvtColor(src=image, code=cv2.COLOR_BGR2GRAY),
                    )
                    kld_score = self._kld(
                        hist1=image_histogram,
                        hist2=self._baseline_histogram,
                    )
